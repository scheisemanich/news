#!/bin/bash
# Single Playlist Update Script with Fixed Paths
# Creates a single playlist on first run and always updates that same playlist afterward

# Configuration - adjust for running from scripts directory
if [[ "$(basename $(pwd))" == "scripts" ]]; then
  # We're in the scripts directory, so go up one level
...
echo "========================================="
echo "Running with project directory: $PROJECT_DIR"
echo ""
...
python "$PROJECT_DIR/scripts/youtube_news_aggregator.py" --load-config "$CONFIG_FILE" --days-back 1 --output-file "$OUTPUT_JSON"
...
if [ -f "$PLAYLIST_ID_FILE" ] && [ -s "$PLAYLIST_ID_FILE" ]; then
    ...
    python "$PROJECT_DIR/scripts/youtube_playlist_creator.py" --json-file "$OUTPUT_JSON" --credentials "$CREDENTIALS_FILE" --playlist-id "$PLAYLIST_ID" --max-per-channel 5
...
OUTPUT=$(python "$PROJECT_DIR/scripts/youtube_playlist_creator.py" --json-file "$OUTPUT_JSON" --credentials "$CREDENTIALS_FILE" --title "$PLAYLIST_TITLE" --description "$PLAYLIST_DESC" --privacy "private" --max-per-channel 5)
